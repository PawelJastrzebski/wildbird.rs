...
use syn::{FnArg, ItemFn, PatType, ReturnType, spanned::Spanned, __private::TokenStream2};
use crate::_utils::*;
...
fn _impl_var_static(fun: &ItemFn, first_arg: Option<&&PatType>, attribute: VarAttr) -> TokenStream2 {
    ...
    let static_name = attribute.name.unwrap_or(function_name.to_string().to_uppercase());

    if !is_async {
        return impl_static(&function_name, &static_name, &return_type, &visibility_token);
    }

    let init_function_name = format_ident!("_{}_init", function_name.to_string()).to_token_stream();
    let mut static_impl = impl_static(&init_function_name, &static_name, &return_type, &visibility_token);
    ...
    if let Some(_callback_arg) = first_arg {
        ...
        static_impl = impl_static(&init_function_name, &static_name, &return_type, &visibility_token);
...
fn _var_validate(
    ...
    let first_arg_type = first_arg.ty.to_token_stream().to_string().trim().replace(" ", "");
    ...
    if fields.len() > 1 {
        return Some(
            error(lazy_fn.sig.inputs.span(), format!("#[var] - Invalid number of arguments\n\t zero or one (callback) allowed"))
        );
    }

    if lazy_fn.sig.asyncness.is_none() {
        return Some(
            error(lazy_fn.sig.span(), format!("#[var] - Callback function must by async"))
        );
    }

    let call_exist = format!("{first_arg_name}.call(");
    if !lazy_fn.block.to_token_stream().to_string().trim().contains(&call_exist) {
        return Some(
            error(lazy_fn.block.span(), format!("#[var] - {first_arg_name}.call(T): method must be called"))
        );
    }
